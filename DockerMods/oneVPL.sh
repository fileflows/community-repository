...
if [ "$1" == "--uninstall" ]; then
    ...
    if apt-get remove -y libmfx-gen1.2 i965-va-driver-shaders intel-media-va-driver-non-free intel-opencl-icd; then
...
if ! apt-get -qq update || ! apt-get install -yqq libmfx-gen1.2 i965-va-driver-shaders intel-media-va-driver-non-free intel-opencl-icd; then
    handle_error
fi
