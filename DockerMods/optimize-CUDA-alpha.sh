# ----------------------------------------------------------------------------------------------------
# Name: Optimize CUDA Alpha
# Description: Installs the alpha of FFmpeg FileFlows Edition + CUDA
# Revision: 1
# Icon: data:image/png;base64,
# ----------------------------------------------------------------------------------------------------

#!/bin/bash

set -e

# --------------- Configurable Paths --------------------
# Constants
FFMPEG_DIR="/usr/lib/ffmpeg-fileflows-edition"
FFMPEG_LINK=/usr/local/bin/ffmpeg
FFMPEG_CUDA_DIR="/app/common/ffmpeg-fileflows-edition-cuda"
FFMPEG_VERSION=7.1.1-alpha2
FFMPEG_TARGET="noble"

# ---------------------- Utility ------------------------
function handle_error {
    echo "An error occurred. Exiting..."
    exit 1
}

# ---------------------- FFmpeg ------------------------
function is_ffmpeg_installed {
    [ -x "$FFMPEG_LINK" ] && "$FFMPEG_LINK" -version 2>/dev/null | grep -q FileFlows
}

function install_ffmpeg {
    if is_ffmpeg_installed; then
        echo "FFmpeg FileFlows Edition is already installed. ($FFMPEG_DIR)"
        return
    fi

    echo "Installing FFmpeg FileFlows Edition..."

    architecture=$(uname -m)
    if [[ ! "$architecture" =~ ^(x86_64|aarch64|armv7l)$ ]]; then
        echo "Unsupported architecture: $architecture"
        exit 1
    elif [[ "$architecture" == aarch64 ]]; then
        TARGET_ARCH="arm64"
    else
        TARGET_ARCH="amd64"
    fi

    apt-get -qq update

    wget --no-verbose -O /tmp/ffmpeg-fileflows-edition.deb "https://github.com/lawrencecurtis/ffmpeg-fileflows-edition/releases/download/v${FFMPEG_VERSION}/ffmpeg-fileflows-edition_${FFMPEG_VERSION}-${FFMPEG_TARGET}_${TARGET_ARCH}.deb"
    apt-get install --no-install-recommends --no-install-suggests -yqq intel-opencl-icd /tmp/ffmpeg-fileflows-edition.deb
    rm /tmp/ffmpeg-fileflows-edition.deb

    ln -sf "$FFMPEG_DIR/ffmpeg" /usr/local/bin/ffmpeg
    ln -sf "$FFMPEG_DIR/ffprobe" /usr/local/bin/ffprobe
    ln -sf "$FFMPEG_DIR/dovi_tool" /usr/local/bin/dovi_tool

    echo "FFmpeg FileFlows Edition installed. ($FFMPEG_DIR)"
}

function uninstall_ffmpeg {
    echo "Uninstalling FFmpeg FileFlows Edition..."
    rm -f /usr/local/bin/ffmpeg /usr/local/bin/ffprobe /usr/local/bin/dovi_tool
    apt-get remove --purge -y ffmpeg-fileflows-edition || echo "Not installed."
}

# --------------------- FFmpeg + CUDA  -----------------------
function is_ffmpeg_cuda_installed {
    [ -x "$FFMPEG_CUDA_DIR/ffmpeg" ] && "$FFMPEG_CUDA_DIR/ffmpeg" -version 2>/dev/null | grep -q FileFlows
}

function install_ffmpeg_cuda {
    if is_ffmpeg_cuda_installed; then
        echo "FFmpeg FileFlows Edition + CUDA is already installed. ($FFMPEG_CUDA_DIR)"
        return
    fi

    echo "Installing FFmpeg FileFlows Edition + CUDA..."

    architecture=$(uname -m)
    if [[ ! "$architecture" =~ ^(x86_64|aarch64|armv7l)$ ]]; then
        echo "Unsupported architecture: $architecture"
        exit 1
    elif [[ "$architecture" == aarch64 ]]; then
        TARGET="linuxarm64-cuda"
    else
        TARGET="linux64-cuda"
    fi

    apt-get -qq update
    wget --no-verbose -O /tmp/ffmpeg-fileflows-edition-cuda.tar.xz "https://github.com/lawrencecurtis/ffmpeg-fileflows-edition/releases/download/v${FFMPEG_VERSION}/ffmpeg-fileflows-edition_${FFMPEG_VERSION}-portable_${TARGET}.tar.xz"
    mkdir -p $FFMPEG_CUDA_DIR
    tar -xvf /tmp/ffmpeg-fileflows-edition-cuda.tar.xz -C $FFMPEG_CUDA_DIR
    rm /tmp/ffmpeg-fileflows-edition-cuda.tar.xz

    echo "FFmpeg FileFlows Edition + CUDA installed.  ($FFMPEG_CUDA_DIR)"
}

function uninstall_ffmpeg_cuda {
    echo "Uninstalling FFmpeg FileFlows Edition + CUDA..."
    rm -rfv $FFMPEG_CUDA_DIR
}

# ---------------------- Main Actions ------------------------
function install_all {
    install_ffmpeg
    install_ffmpeg_cuda
    echo "✅ All components installed."
}

function uninstall_all {
    uninstall_ffmpeg
    uninstall_ffmpeg_cuda
    echo "🗑️ All components uninstalled."
}

# ---------------------- Entrypoint ------------------------
if [ "$1" == "--uninstall" ]; then
    uninstall_all
else
    install_all
fi

exit 0